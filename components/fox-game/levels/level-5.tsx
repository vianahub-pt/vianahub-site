...
import { Fox } from "../fox"
import { level5MazeDesktop, level5MazeMobile, type Position } from "../../../data/fox-game/mazes-level-5"
...
export function Level5({
  ...
  const maze = isMobile ? level5MazeMobile : level5MazeDesktop
  ...
  const getCellClass = (x: number, y: number) => {
    ...
    if (gameStarted && cellValue !== 1 && isReachable(x, y)) {
      baseClass +=
        " cursor-pointer hover:bg-gray-200 hover:ring-2 hover:ring-gray-400 transition-all duration-200 touch-manipulation"
    }
  ...
  return (
    <motion.div
      ...
      className={`relative border-4 border-gray-600 rounded-lg overflow-hidden bg-gray-50 ${isMobile ? "mx-auto" : ""}`}
      style={{
      ...
      >
        {maze.grid.flat().map((cell, index) => {
          ...
          return (
            <div
              key={index}
              className={`border border-gray-200 ${getCellClass(x, y)} relative overflow-hidden`}
              style={{
                width: cellSize,
                height: cellSize,
            ...
            >
              ...
              {gameStarted && isReachable(x, y) && maze.grid[y][x] !== 1 && (
                <div className="absolute inset-0 bg-gray-400 bg-opacity-25 pointer-events-none" />
              )}
            </div>
          )
        })}
