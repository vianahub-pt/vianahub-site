...
import { level5DesktopMaze, level5MobileMaze, type Position } from "../../../data/fox-game/mazes-level-5"
...
export function Level5({
  ...
  const maze = isMobile ? level5MobileMaze : level5DesktopMaze
  ...
  const getCellStyle = (x: number, y: number) => {
    ...
    let className = "border border-gray-300 relative cursor-pointer transition-all duration-200 "

    if (isFoxPosition) {
      className += "ring-2 ring-slate-500 ring-offset-1 "
    } else if (isEnd) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3 }}
      className="bg-gray-100 border-4 border-gray-400 rounded-lg p-2 shadow-lg"
    >
      <div
