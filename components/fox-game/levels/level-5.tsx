...
import {
  ...
  level5MazeMobile,
  type Position,
} from "../../../data/fox-game/mazes-level-5";
...
export function Level5({
  ...
  return (
    <motion.div
      ...
      className={`relative border-4 border-gray-600 rounded-lg overflow-hidden bg-gray-50 ${
        isMobile ? "mx-auto" : ""
      }`}
      style={{
      ...
      >
        {maze.grid.flat().map((cell, index) => {
          ...
          return (
            <div
              key={index}
              className={`border border-gray-200 ${getCellClass(
                x,
                y
