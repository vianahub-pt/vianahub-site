...
import { Fox } from "../fox"
import { level2MazeDesktop, level2MazeMobile, type Position } from "../../../data/fox-game/mazes-level-2"
...
export function Level2({
  ...
  const maze = isMobile ? level2MazeMobile : level2MazeDesktop
  ...
  const getCellClass = (x: number, y: number) => {
    ...
    switch (cellValue) {
      ...
      case 3:
        ...
        break
      default:
        baseClass = "bg-orange-100" // Caminho livre
        break
    }
    ...
    if (gameStarted && cellValue !== 1 && isReachable(x, y)) {
      baseClass +=
        " cursor-pointer hover:bg-orange-200 hover:ring-2 hover:ring-orange-400 transition-all duration-200 touch-manipulation"
    }
  ...
  return (
    <motion.div
      ...
      className={`relative border-4 border-orange-600 rounded-lg overflow-hidden bg-orange-50 ${isMobile ? "mx-auto" : ""}`}
      style={{
      ...
      >
        {maze.grid.flat().map((cell, index) => {
          ...
          return (
            <div
              key={index}
              className={`border border-orange-200 ${getCellClass(x, y)} relative overflow-hidden`}
              style={{
                width: cellSize,
                height: cellSize,
