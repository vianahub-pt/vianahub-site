...
import { level2DesktopMaze, level2MobileMaze, type Position } from "../../../data/fox-game/mazes-level-2"
...
export function Level2({
  ...
  const maze = isMobile ? level2MobileMaze : level2DesktopMaze
  ...
  const getCellStyle = (x: number, y: number) => {
    ...
    let className = "border border-orange-300 relative cursor-pointer transition-all duration-200 "

    if (isFoxPosition) {
      className += "ring-2 ring-red-500 ring-offset-1 "
    } else if (isEnd) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3 }}
      className="bg-orange-100 border-4 border-orange-400 rounded-lg p-2 shadow-lg"
    >
      <div
          ...
          row.map((cell, x) => {
            if (cell === 1) {
              ...
              return (
                <div
                  key={`${x}-${y}`}
                  className="bg-orange-800 border border-orange-900"
