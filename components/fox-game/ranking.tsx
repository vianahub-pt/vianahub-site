...
export function Ranking({ ranking, currentPlayerName, currentTime }: RankingProps) {
  ...
  return (
    <Card className="w-full max-w-md bg-white/90 backdrop-blur-sm shadow-lg">
      <CardHeader>
        <CardTitle className="text-xl font-bold text-amber-800 text-center">🏆 Ranking dos Melhores Tempos</CardTitle>
      </CardHeader>
      <CardContent>
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {ranking.length === 0 ? (
            <div className="text-center py-8 text-amber-600">
              <p className="text-lg">🌟 Seja o primeiro no ranking!</p>
              <p className="text-sm mt-2">Complete todos os níveis para aparecer aqui</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-amber-800">
                        {entry.name}
                        {isCurrentPlayer && (
                          <span className="ml-2 text-xs bg-green-600 text-white px-2 py-1 rounded">Você</span>
                        )}
                      </div>
